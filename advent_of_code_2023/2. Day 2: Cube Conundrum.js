...
const str = `Game 1: 4 red, 3 blue; 6 blue, 16 green; 9 blue, 13 green, 1 red; 10 green, 4 red, 6 blue
...
const str1 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;

const str2 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;
...
function calc(s) {
  ...
  return arr.reduce((acc, val) => {
    const gameIndex = +val.match(/(\d+)/)[0];
    const blue = val.match(/(\d+)\s*(blue)/g);
    const green = val.match(/(\d+)\s*(green)/g);
    const red = val.match(/(\d+)\s*(red)/g);

    function findIfPossible(color, max) {
      return color.every(c => {
        const n = +c.match(/(\d+)/g)[0];

        return n <= max;
      });
    }

    const b = findIfPossible(blue, 14);
    const g = findIfPossible(green, 13);
    const r = findIfPossible(red, 12);

    return acc + (b && g && r ? gameIndex : 0);
  }, 0);
}
...
function calc2(s) {
  ...
  return arr.reduce((acc, val) => {
    ...
    const bi = findMax(blue);
    const gi = findMax(green);
    const ri = findMax(red);

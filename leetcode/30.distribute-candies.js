...
const distributeCandies = function(candyType) {
    // as set does not contain duplicates, we could compare size of set and half of array length and return the smallest
    const typesOfCandies = new Set(candyType).size;
    const candiesToEat = candyType.length / 2;

    return Math.min(typesOfCandies, candiesToEat);
};
